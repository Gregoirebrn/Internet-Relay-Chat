...
int Channel::Kick(const std::string& buff, int fd_cli)
{
	if (std::string::npos == buff.find(' '))
		return (SendMessage(fd_cli, ERR_NEEDMOREPARAMS("KICK")), 461);
	if (std::string::npos == buff.find('#'))
		return (SendMessage(fd_cli, ERR_NOSUCHCHANNEL(buff.substr(0, buff.find(' ')))), false);
	...
	if (!CheckRights(_client->GetName(fd_cli), channel, fd_cli))// check if the client caller have the rights to kick
		return (404);
	if (nick == "bot") {
...
//	SendMessage(_client->GetFd(nick), RPL_KICKED(_client->GetName(fd_cli), channel, nick));
	return (0);
}